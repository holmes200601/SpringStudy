...
public class TimeUtils {
    ...
    public static int calculateDiffHours(Date d1, Date d2) {
        ...
        int result = (365 * 24 * (d2year - d1year)) + (24 * (d2day - d1day)) + (d2hour - d1hour)
    ...
    public static Date calMonthStart(int year, int month) {
        Calendar cal = TimeUtils.getCurrentCalendar();
    ...
    public static Date calMonthEnd(int year, int month) {
        Calendar cal = TimeUtils.getCurrentCalendar();
        cal.set(year, month - 1, 1, 0, 0, 0);
        cal.setTime(DateUtils.addMonths(cal.getTime(), 1));

        return DateUtils.addSeconds(cal.getTime(), -1);
    }
}
